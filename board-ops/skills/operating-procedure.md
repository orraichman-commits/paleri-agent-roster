...
| Situation | Action |
|---|---|
| Board trigger / CEO or Owner requests a pack | Gather the four views; build the Board Pack |
...
| Evidence covers a single period only | Recommend freeze at most; never retire on one period |
...
- One quiet period on an agent → report low utilization for that period; withhold any retire
...
5. Is any retire recommendation backed by more than one period?
