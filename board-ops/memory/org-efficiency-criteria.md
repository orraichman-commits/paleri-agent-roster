...
- **On-demand agents are not idle.** `strategic-intelligence-agent` is scheduled/on-demand by
  design. Silence from it in a period is expected and is never a finding.
...
| Retire | Sustained non-value across **more than one** period, with routing ruled out as the cause. |
